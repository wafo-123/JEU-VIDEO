...
const userExist = async (name, password) => {
  ...
  const signInFirebase = async(username, password)=>{
    var idUser = userExist(username, password);
    if(idUser==""){
        var newuser = createUser(username, password);
        createDocument("users",newuser);
        alert("Bravo vous etes mtn inscrit")
...
const getDataFirebase = async() => {
    ...
    const collection = await getDocument("categorie");
    ...
    collection.forEach((jeu , index) => {
        ...
        <div class="flex relative">
          ...
          <div class="px-8 py-10 relative z-10 w-full border-4 border-gray-200 bg-white opacity-0 hover:opacity-100">
            ...
            <p>${jeu.description}</p>
            <a href="${jeu.lien}"  class="bg-red-500 rounded-m p-2 mt-50 ms-40 text-white">en savoir plus</a>
          </div>
        </div>`
    });
